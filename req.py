...
def getAllFileNumber(cikNumber):
    """getAllFileNumber

    Get the File number (Filing number) of all 13-f from a given company based on CIK
...
def getTextFile(fileIndex):
    ...
    Get the text file from a given file index from the SEC.gov website with built-in query timer as mandated of 10 search per second

    Params:
        file index (String) : The file index to be searched
    Return:
        text file (String)  : The content of the text file
    """
    start = time.time_ns()
